...
async function tlExtractTopic(title, llmFn) {
  // "OO 씨 구속"처럼 사건을 중심으로 뽑으면, 그 사건 자체(예: 구속)로 검색어가 좁아져서
  // 정작 그 사건이 애초에 왜 시작됐는지(예: 계엄령 선포)는 "구속"이라는 단어를 안 쓰는
  // 보도라 과거 검색에 안 걸릴 수 있다 -- 실측: "尹 구속"으로 검색하니 尹 본인이 아니라
  // 다른 인물들의 "구속" 보도만 잔뜩 잡히고, 정작 이 사태의 출발점은 안 걸렸음. 특정 인물/
  // 조직을 둘러싼 진행 중인 사태라면 그 사건이 아니라 인물/조직 자체를 중심으로 뽑아서
  // 사태 전체(출발점 포함)가 검색되게 한다.
  const prompt =
    '다음은 오늘 올라온 기사 제목입니다. 이 기사가 다루는 오늘자 세부 사실이 아니라, ' +
    '이 기사가 속한 더 큰 사건·이슈 자체를 뉴스 검색에 쓸 짧은 구절(5단어 이내)로 뽑으세요. ' +
    '이 기사가 특정 인물이나 조직을 둘러싼 진행 중인 사태(수사·재판·정치적 위기 등)의 일부라면, ' +
    '오늘 기사의 구체적인 혐의·절차가 아니라 그 인물/조직 자체를 중심으로 뽑으세요 -- 그래야 이 ' +
    '사태가 애초에 왜 시작됐는지까지 과거 검색에 걸립니다 ' +
    '(예: "尹 영치금 17억 논란"이나 "尹 구속"이 아니라 "尹 대통령"처럼). ' +
    '인물/조직이 뚜렷하지 않은 단발성 사건이면 그 사건 자체를 뽑으세요. ' +
    '다른 설명 없이 검색어만 답하세요.\n\n제목: ' + title;
...
async function refreshTimelineForPost(supabase, post) {
  ...
  // found는 TIMELINE_WINDOWS 순서(최근 구간부터)를 그대로 물려받아서, 옛날 순서로 안 정렬하고
  // 그냥 25개로 자르면 최근 구간 후보만으로 25개가 다 차버려 정작 근본 원인이 있는 가장 오래된
  // 구간(365~730일 전)이 통째로 밀려날 수 있다 -- 오래된 것부터 채워서 뿌리가 되는 사건이
  // 잘리지 않게 한다.
  const byOldestFirst = [...deduped].sort((a, b) => new Date(a.item.pubDate) - new Date(b.item.pubDate));
  const candidates = byOldestFirst.slice(0, 25);
  const listing = candidates.map((c, i) =>
    '[' + i + '] ' + new Date(c.item.pubDate).toISOString().slice(0, 10) + ' - ' + c.item.title
  ).join('\n');
  const prompt =
    '다음은 "' + rawTitle + '" 기사와 관련된 과거 보도 후보 목록입니다(번호, 날짜, 제목, 오래된 순).\n\n' + listing +
    '\n\n이 중에서 이 사건의 전후 맥락을 이해하는 데 정말 중요한 분기점만 5~8개 고르세요. ' +
    '특히 이 사건이 애초에 왜, 어떻게 시작됐는지 보여주는 가장 근본적인 출발점(목록에서 가장 오래된 ' +
    '축에 속할 가능성이 높음)과, 그 이후 국면을 크게 바꾼 주요 전환점을 우선하세요. 단순 반복되는 ' +
    '절차 보도(예: 영장 연장, 소환조사, 공판기일 등 사소한 후속보도)나 중복·무관한 기사는 빼세요. ' +
    '각각을 한국어 한 문장으로 짧게 요약하세요(날짜는 이미 있으니 요약 문장에 다시 쓰지 마세요). ' +
